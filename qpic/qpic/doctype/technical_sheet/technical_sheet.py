...
@frappe.whitelist()
def get_raw_materials_itemwise(item_code, exchange_rate=None):
	exchange_rate = flt(exchange_rate) or 1

	item = frappe.db.get_value(
		"Item",
		item_code,
		["item_name", "stock_uom", "valuation_rate", "description"],
		as_dict=1
	)
	...
	usd_rate = base_rate / exchange_rate
...
def get_overheads_and_others(operation, hrsmt, workstation=None, lamination_side=None, lamination_width=None, fabric_type=None,exchange_rate=None):
	...
	items = frappe.db.get_all(
		"Item",
		{"operation": operation},
		["name", "item_name", "stock_uom", "valuation_rate", "description", "operation","item_group"]
	)
	data = []
	exchange_rate = flt(exchange_rate) or 1
	for item in items:
...
def make_costing_sheet(source_name, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		pcs_mt = 1000000 / source.bag_weight if source.bag_weight > 0 else 0
		target.pcs_mt = pcs_mt
		target.mesh = str(round(source.tape_warp_mesh)) + " X " + str(round(source.tape_weft_mesh))
		target.tape_width = str(flt(source.warp_tape_width, 2)) + " X " + str(flt(source.weft_tape_width, 2))
		...
		if source.technical_sheet_type=="Fabric":
			...
			target.mt_cnt_by_product = source.mt___container
		elif source.technical_sheet_type=="Small Bag":
			target.conversion=source.convsn_quot_usdmt
			target.mt_cnt_by_product = source.pieces__container / pcs_mt
...
@frappe.whitelist()
def update_overhead_item_price():
	group_name = ["Tape","Loom"]
	oh_group=["Tape","SB Fabric","FIBC fabric","Lamination","PE"]
	item = frappe.db.get_value(
		"Item",
		{"item_group": "Overhead", "item_name": ["like", f"%{group_name}%"]},
		["name"],
		as_dict=1
	)
	if not item:
		frappe.throw("Item not found for Overhead group")
	workstation = frappe.db.get_value(
		"Workstation",
		{"name": ["like", f"%{group_name}%"]},
		["name", "no_of_machine"],
		as_dict=1
	)
	if not workstation or not workstation.no_of_machine:
		frappe.throw("Workstation or number of machines not found")
	overhead_summary = frappe.get_doc("Overhead Summary")
	total_value = 0
	for row in overhead_summary.hour_data:
		if row.group == group_name:
			total_value = row.total
			break
	if not total_value:
		frappe.throw("Total not found in Overhead Summary")
	item_price = total_value / workstation.no_of_machine
	frappe.db.set_value("Item Price", {"item_code":item.name}, "price_list_rate", item_price)
	return {
		"item": item.name,
