...
class DetailedTechnicalSheet(Document):
	def on_submit(self):
		for opp_item in self.technical_costing_item:
			tc_id = frappe.db.exists('Sub Commercial Costing',{'name': self.name,'opportunity':self.opportunity}, ['*'])
			if not tc_id:
				tc = frappe.new_doc("Sub Commercial Costing")
				...
				if self.needle_loom_material_combination:
					for nl in self.needle_loom_material_combination:
						tc.append("needle_loom_raw_material",{
								"item_code": nl.item_code,
								"item_description": nl.item_description,
								"dosage": nl.dosage,
								"qty": nl.qty,
								"unit": nl.unit,
							})
