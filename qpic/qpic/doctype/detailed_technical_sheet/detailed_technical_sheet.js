...
frappe.ui.form.on('Detailed Technical Sheet', {
	onload(frm){
				...
				if(r.message){
					...
					$.each(r.message,function(i,d){
						frm.add_child('payment_schedule',{
							...
							'base_payment_amount':d.base_payment_amount,
		...
		frappe.db.get_value('Technical Sheet FIBC',frm.doc.technical_sheet_fibc,['notes'])
		.then(r => {
			var value = r.message.notes
			frm.set_value('notes',value)
		})
		frappe.db.get_value('Technical Sheet FIBC',frm.doc.technical_sheet_fibc,['delivery_term'])
		.then(r => {
			var value = r.message.delivery_term
			frm.set_value('delivery_term',value)
		})		
	},
	opportunity(frm){
		frappe.call({
			...
			callback: function(r) {
				if(r.message){
					$.each(r.message[0],function(i,d){			
						frm.set_value('order_quantity',d.qty)	
						frm.set_value('item_code',d.item_code)	
						frm.set_value('item_name',d.item_name)	
						frm.set_value('item_group',d.item_group)	
						frm.set_value('uom',d.uom)	
						frm.clear_table('technical_costing_item')
						frm.add_child('technical_costing_item',{
							item_code:d.item_code,
							item_name:d.item_name,
							item_group:d.item_group,
							sub_group:d.sub_group,
							qty_as_per_stock_uom:d.qty_as_per_stock_uom,
							conversion_factor:d.conversion_factor,
							uom:d.uom,
							qty:d.qty,
							sales_person:r.message[1]

						})
						frm.refresh_field('technical_costing_item')
				})
				}
			}
		});
	...
	refresh(frm){
		...
		function () {
			window.location.href = "/app/technical-sheet-fibc/" + frm.doc.technical_sheet_fibc;
		});
	},
	set_css(frm){
		...
		$('input[data-fieldname="strip2_output_kghour"]').css("background-color","#deeefa")
		$('input[data-fieldname="strip2_width"]').css("background-color","#deeefa")
		$('input[data-fieldname="strip2_colour"]').css("background-color","#deeefa")
		...
		$('input[data-fieldname="nl_belt_width_cm"]').css("background-color","#deeefa")
		$('input[data-fieldname="nl_cut_length_cm"]').css("background-color","#deeefa")
		$('input[data-fieldname="nl_weight_pcs_gm"]').css("background-color","#deeefa")
		$('input[data-fieldname="order_requirement_mtr"]').css("background-color","#deeefa")
		...
		$('input[data-fieldname="nl_wastage_provision_finishing"]').css("background-color","#f5ece4")
		$('input[data-fieldname="nl_efficiency"]').css("background-color","#f5ece4")
		$('input[data-fieldname="nl_mtr__hour_target"]').css("background-color","#f5ece4")
	...
	validate(frm) {
		...
		if(frm.doc.nl_material_combination == 1){
			if(frm.doc.needle_loom_material_combination){
				var dosage = 0
				$.each(frm.doc.needle_loom_material_combination, function (i, d) {
					dosage += d.dosage
					console.log(dosage)
					frm.set_value("total_nl",dosage)
				})
				if(frm.doc.total_nl ==100){
					frappe.validated = true;
				}
				else{
					frappe.throw("Needle Loom material combination dosage should be 100")
				}
			}
		}
	...
	top(frm){
		...
		if(frm.doc.top){
			...
			else if(frm.doc.top == "FIBC") {
    			frappe.db.get_single_value('Technical Costing Master', 'fibc_wastage')
    				.then(r => {
    					frm.set_value('loom_wastage_provision_finishing', r)
    			})
    	    }
		...
		if(frm.doc.top){
    	    if(frm.doc.top == "Heat Cut") {
    			...
    			frappe.db.get_single_value('Technical Costing Master', 'fibc_extra_to_cut_length')
    				.then(r => {
    					frm.set_value('top_length', r)
    			})
    	    }
    	...
    	if(frm.doc.top){
    	    if(frm.doc.top == "Heat Cut") {
    			...
    			frappe.db.get_single_value('Technical Costing Master', 'fibc_thread')
    				.then(r => {
    					frm.set_value('top_thread', r)
    			})
    	    }
	...
	bottom(frm){
		...
		if(frm.doc.bottom){
			...
			else if(frm.doc.bottom == "FIBC") {
    			frappe.db.get_single_value('Technical Costing Master', 'bottom_fibc_extra_to_cut')
    				.then(r => {
    					frm.set_value('bottom_length', r)
    			})
    	    }
    	...
    	if(frm.doc.bottom){
    	    if(frm.doc.bottom == "SFSS") {
    			...
    			frappe.db.get_single_value('Technical Costing Master', 'bottom_fibc_thread')
    				.then(r => {
    					frm.set_value('bottom_thread', r)
    			})
    	    }
	...
	loom_picks_per_min(frm){
		...
		var loom_order_requirement = Math.ceil(((frm.doc.production_size_cut_leng_y * frm.doc.order_quantity)/100) * (1+ frm.doc.loom_wastage_provision_finishing/100))
	...
	weft_output_kghour(frm){
		...
		var weft_order_requirement = ((frm.doc.fabric_gms*frm.doc.order_quantity)/1000/2) * (1 + frm.doc.weft_wastage_provision_loom/100)
	...
	warp_output_kghour(frm){
		...
		var warp_order_requirement = (((frm.doc.fabric_gms*frm.doc.order_quantity)/1000/2) * (1 + frm.doc.warp_wastage_provision_loom/100)) - frm.doc.strip_order_requirement
	...
	stitching_bags_manhour(frm){
		frm.set_value('stitching_order_requirement',frm.doc.order_quantity)
		frm.set_value('stitching_manhour',frm.doc.stitching_order_requirement / frm.doc.stitching_bags_manhour)
	},
	needleloom(frm){
		var nl = (frm.doc.nl_weight_pcs_gm / (frm.doc.nl_cut_length_cm * frm.doc.nl_belt_width_cm))*(100*100)
		frm.set_value('nl_gsm',nl)
		var wt = (frm.doc.nl_weight_pcs_gm /frm.doc.nl_cut_length_cm *100)
		frm.set_value('nl_linear_meter_wt_gm',wt)
		var oq = (frm.doc.order_quantity *frm.doc.nl_wastage_provision_finishing/100) + frm.doc.order_quantity
		frm.set_value('nl_order_requirement',oq)
		var fin = frm.doc.nl_order_requirement / frm.doc.nl_mtr__hour_target
		frm.set_value('nl_machine_hours',fin)


	},
	nl_belt_width_cm(frm){
		frm.trigger('needleloom')
	},
	nl_cut_length_cm(frm){
		frm.trigger('needleloom')
	},
	nl_weight_pcs_gm(frm){
		frm.trigger('needleloom')
	},
	nl_wastage_provision_finishing(frm){
		frm.trigger('needleloom')
	},
	nl_mtr__hour_target(frm){
		frm.trigger('needleloom')
	},
	nl_efficiency(frm){
		frm.trigger('needleloom')
	...
	liner_output_kghour(frm){
		...
		var liner_order_requirement = (((frm.doc.liner_weight_pcs * frm.doc.order_quantity)/1000) * (1+ frm.doc.liner_wastage_provision/100))
		...
		frm.set_value('order_requirement_liner',frm.doc.order_quantity)
		...
		var order_requirement_mtr = (frm.doc.order_quantity * (frm.doc.liner_size_cut_length/100))*(1+frm.doc.liner_wastage_provision/100)
	...
	cutting_machine_speed_bagsmin(frm){
		...
		frm.set_value('cutting_order_requirement',frm.doc.order_quantity)
...
frappe.ui.form.on('Needle Loom Material Combination', {
	dosage: function(frm,cdt,cdn){
		var nl_mat_comb = frm.doc.nl_order_requirement * (1 + (frm.doc.nl_wastage_provision_finishing/100))
		// console.log(liner_mat_comb)
		var row = locals[cdt][cdn]
		var qty = ((row.dosage/100)*nl_mat_comb)
		row.qty = qty
		frm.refresh_field("needle_loom_material_combination")
	}
})
