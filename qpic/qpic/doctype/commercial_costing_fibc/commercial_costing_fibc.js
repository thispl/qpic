...
frappe.ui.form.on('Commercial Costing FIBC', {
	cost(frm){
	...
	refresh(frm){
		frm.add_custom_button(__("Print"), function () {
			...
			var print_format = "Commercial Costing FIBC";
			window.open(frappe.urllib.get_full_url("/api/method/frappe.utils.print_format.download_pdf?"
				+ "doctype=" + encodeURIComponent("Commercial Costing FIBC")
	...
	dos_calc(frm){
		$.each(frm.doc.weft_raw_material,function(i,d){
			d.dosage = d.qty / frm.doc.total_quantity_weft
			d.dosage = d.dosage*100
			d.rate = d.amount /d.qty
		})
		frm.refresh_field("weft_raw_material")

		$.each(frm.doc.warp_raw_material,function(i,d){
			d.dosage = d.qty / frm.doc.total_quantity_warp
			d.dosage = d.dosage*100
			d.rate = d.amount /d.qty
		})
		frm.refresh_field("warp_raw_material")
		...
		$.each(frm.doc.strip_raw_material,function(i,d){
			d.dosage = d.qty / frm.doc.total_quantity_strip
			d.dosage = d.dosage*100
			d.rate = d.amount /d.qty
		})
		frm.refresh_field("strip_raw_material")
		...
		$.each(frm.doc.liner_raw_material,function(i,d){
			d.dosage = d.qty / frm.doc.total_quantity_liner
			d.dosage = d.dosage*100
			d.rate = d.amount /d.qty
		})
		frm.refresh_field("liner_raw_material")
		...
		$.each(frm.doc.needle_loom_raw_material,function(i,d){
			d.dosage = d.qty / frm.doc.total_quantity_nl
			d.dosage = d.dosage*100
			d.rate = d.amount /d.qty
		})
		...
		$.each(frm.doc.lamination_raw_material,function(i,d){
			d.dosage = d.qty / frm.doc.total_quantity_lamination
			d.dosage = d.dosage*100
			d.rate = d.amount /d.qty
		})
		frm.refresh_field("lamination_raw_material")
