...
frappe.ui.form.on('Commercial Costing Fabric', {
	cost1(frm){
	...
	refresh(frm){
		frm.add_custom_button(__("Print"), function () {
			...
			var print_format = "Commercial Costing Fabric";
			window.open(frappe.urllib.get_full_url("/api/method/frappe.utils.print_format.download_pdf?"
				+ "doctype=" + encodeURIComponent("Commercial Costing Fabric")
	...
	onload(frm){
		...
		if(!frm.doc.__islocal){
			...
			frappe.db.get_single_value('Commercial Costing Master', 'loom_overhead_rate')
				.then(r => {
